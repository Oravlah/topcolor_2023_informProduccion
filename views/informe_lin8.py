...
dcc.Location(id='url_rehacer'+linea, refresh=True),
                ...
                dbc.Col([dcc.Input(value='---', type='text', id='id_lin'+linea, disabled=True,
                                style={'margin-top': '5px', "width": "20%",
                                       'background-color': 'black', 'color': 'white',
                                       'text-align': 'center', 'font-size': '20px'})]),
             dbc.Col([html.H3('Informe Línea Aditivo' )]),
...
def revisar_informelinea(n_clicks, id_rep, linea):
    ...
    try:
        if n_clicks > 0:
            ...
            linea_prueba='8'  ########################################   LINEA SOLO DE PRUEBA SE DEBE CAMBIAR POR EL PARAMETRO LINEA

            url = f"http://{host}:{port_reporte}/infoLinea/&{linea_prueba}/&{id_rep}"
            print(url)
            ...
            return (dbc.Card([
                    ...
                    dbc.CardBody([dbc.Row([dbc.Col([dcc.Input(value=f'Línea Aditivo', type='text', id='nom_linea_det', disabled=True, style={"width": "100%", 'background-color': 'black','color':'white','text-align': 'center','font-size': '25px'})])]),
                        dbc.Row([dbc.Col([
                                dbc.Card([
